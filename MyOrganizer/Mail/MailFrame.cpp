...
BOOL CMailDialogBar1::OnInitDialogBar()
{
	...
	if (pLayout != NULL)
	{
		pLayout->AddAnchor (IDC_MAIL_EDIT_FROM, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
		pLayout->AddAnchor (IDC_MAIL_EDIT_TO, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
		pLayout->AddAnchor (IDC_MAIL_EDIT_CC, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
		pLayout->AddAnchor (IDC_MAIL_EDIT_SUBJECT, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
	}
...
void CMailFrame::CreateFontsList ()
{
...

	{
		...
		item.m_Caption = _T("Calibri");
		lstrcpy (item.m_Font[0].lfFaceName, _T("Calibri"));
		lstrcpy (item.m_Font[1].lfFaceName, _T("Calibri"));
...

	{
		...
		item.m_Caption = _T("Arial - Times New Roman");
		lstrcpy (item.m_Font[0].lfFaceName, _T("Arial"));
...

	{
		...
		item.m_Caption = _T("Times New Roman - Arial");
		lstrcpy (item.m_Font[0].lfFaceName, _T("Times New Roman"));
		lstrcpy (item.m_Font[1].lfFaceName, _T("Arial"));
...
void CMailFrame::AddTab_Options ()
{
	...
	pListBtn->AddGroup (_T("Built-In"), IDB_THEME_COLORS, 103, m_arColors);
	...
	pListBtn->AddGroup (_T("Built-In"), IDB_THEME_EFFECTS, 64, m_arEffects);
	pListBtn->SetIconsInRow (4);
	pListBtn->EnableMenuResize ();
	pPanelThemes->Add (pListBtn);
