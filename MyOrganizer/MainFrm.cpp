...
const COLORREF clrWatermakrBase = RGB(0, 0, 192);
...
BOOL CMainFrame::CreateRibbonBar ()
{
	...
	m_wndRibbonBar.AddToTabs (new CBCGPRibbonButton (ID_APP_ABOUT, _T(""), m_PanelImages.ExtractIcon (0), FALSE, NULL, TRUE, TRUE));
...
BOOL CMainFrame::CreateStatusBar ()
{
	...
	m_wndStatusBar.AddElement (new CBCGPRibbonStatusBarPane (
		ID_STATUSBAR_REFRESH, _T("Refresh"), FALSE, hIconRefresh, NULL, TRUE), 
		_T("Refresh"));
...
void CMainFrame::AddMainCategory ()
{
	...
	{
		CBCGPRibbonBackstageViewPanel* pBackstagePanel = m_wndRibbonBar.AddBackstageCategory (
			_T("File"), IDB_FILE_SMALL);

		pBackstagePanel->AddCommand (ID_FILE_SAVE, _T("&Save"), 2);
		pBackstagePanel->AddCommand (ID_FILE_SAVE_AS, _T("Save &As"), 3);
		pBackstagePanel->AddCommand (ID_FILE_NEW, _T("&New"), 0);
		pBackstagePanel->AddCommand (ID_FILE_OPEN, _T("&Open"), 1);
		pBackstagePanel->AddCommand (ID_FILE_CLOSE, _T("&Close"), 8);

		CBCGPRibbonBackstageViewItemForm* pFormInfo = new CBCGPRibbonBackstageViewItemForm (IDD_BS_FORM_INFO, RUNTIME_CLASS(CBackStagePageInfo));
		pFormInfo->SetWaterMarkImage(IDB_BS_WATERMARK, RGB(0, 0, 192));
		pBackstagePanel->AddView (0, _T("Info\ni"), pFormInfo);

		CBCGPRibbonBackstageViewItemForm* pFormPrint = 
			new CBCGPRibbonBackstageViewItemForm (IDD_BS_FORM_PRINT, RUNTIME_CLASS(CBackStagePagePrint));
		pBackstagePanel->AddView (ID_FILE_PRINT, _T("Print\np"), pFormPrint);

		pBackstagePanel->AddCommand (ID_TOOLS_OPTIONS, _T("Opt&ions"), 26);
		pBackstagePanel->AddCommand (ID_APP_EXIT, _T("E&xit"), 27);
	}
}
...
CBCGPRibbonCategory* CMainFrame::AddTab_Macros ()
{
	...
	pPanelBM->Add (new CBCGPRibbonButton (ID_EDIT_TOGGLEBOOKMARK, _T("Toggle Bookmark"), 3, 1));
	...
	pPanelView->AddSeparator();
	pPanelView->Add (new CBCGPRibbonCheckBox (ID_EDIT_LINENUMBERS, _T("Line Numbers")));
	pPanelView->Add (new CBCGPRibbonCheckBox (ID_EDIT_VIEWWHITESPACE, _T("View White Spaces")));
...
void CMainFrame::OnBeforeChangeVisualTheme(CBCGPWinApp::BCGP_VISUAL_THEME theme)
{
	...
	m_wndRibbonBar.SetBackstageMode(bIsOffice2010Look || bIsOffice2013Look || bIsOffice2016Look);
	...
	if (bIsOffice2013Look || bIsOffice2016Look)
	{
		if (theApp.m_nBackgroundImage != 0)
		{
			...
			else if (theme == CBCGPWinApp::BCGP_VISUAL_THEME_OFFICE_2016_BLACK)
			{
				background.AddaptColors(RGB(255, 255, 255), RGB(90, 90, 90));
			}
...
{
	m_wndCaptionBar.SetCaptionFont(CBCGPVisualManager::GetInstance()->UseLargeCaptionFontInDockingCaptions());
	...
	if (pBackstagePanel != NULL)
	{
		...
		for (int i = 0; i < nViewsCount; i++)
		{
			...
			pForm->SetWaterMarkImage(nWaterMarkResID, clrWatermakrBase);
		}
	}
}
...
CBCGPToolBarImages* CMainFrame::GetTooltipImage (UINT uiID)
{
	...
	BOOL bIsOffice2007Look = (theApp.GetVisualTheme() >= CBCGPWinApp::BCGP_VISUAL_THEME_OFFICE_2007_BLUE && theApp.GetVisualTheme() <= CBCGPWinApp::BCGP_VISUAL_THEME_OFFICE_2007_BLUE) || theApp.GetVisualTheme() == CBCGPWinApp::BCGP_VISUAL_THEME_CUSTOM;
	UINT uiResID = 0;
	...
	if (nIDEvent == IdShowProgressTimer)
	{
		...
		if (m_nProgressValue > 100)
		{
			...
			if (!m_bInfiniteProgressMode)
			{
				...
				params.m_strText = _T("Sender: John Smith\r\nHello, I'm sending you the requested information. Please follow the link below to open the document");
...
void CMainFrame::ShowOptions (int nPage)
{
	...
	propSheet.SetLook (CBCGPPropertySheet::PropSheetLook_List, globalUtils.ScaleByDPI(150));
...
void CMainFrame::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
	...
	OnAfterChangeVisualTheme(theApp.GetVisualTheme());
}
