...
IMPLEMENT_SERIAL(CGanttDialogBar, CEditDialogBar, VERSIONABLE_SCHEMA | 1)

CGanttDialogBar::CGanttDialogBar ()
	: m_bFirstTime (TRUE)
{
}
...
BEGIN_MESSAGE_MAP(CGanttDialogBar, CEditDialogBar)
END_MESSAGE_MAP()

void CGanttDialogBar::DoDataExchange(CDataExchange* pDX)
{
	...
	if (m_bFirstTime)
	{
		...
		if (m_bExtended)
		{
			...
			nFlags = CBCGPDateTimeCtrl::DTM_CHECKED | CBCGPDateTimeCtrl::DTM_SPIN;
			//-------------------
			// Setup date fields:
			//-------------------
...
BOOL CGanttDialogBar::OnInitDialogBar()
{
	...
	CBCGPStaticLayout* pLayout = DYNAMIC_DOWNCAST(CBCGPStaticLayout, GetLayout ());
	if (pLayout != NULL)
	{
		pLayout->AddAnchor (IDC_GANTT_EDIT_NAME, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
		pLayout->AddAnchor (IDC_GANTT_EDIT_SEPARATOR, CBCGPStaticLayout::e_MoveTypeNone, CBCGPStaticLayout::e_SizeTypeHorz);
	}
...
{
	m_wndDialogBar.m_strName  = m_Item.GetName ();
	m_wndDialogBar.m_dtStart  = m_Item.GetStartTime ();
	m_wndDialogBar.m_dtFinish = m_Item.GetFinishTime ();
	m_wndDialogBar.m_lProgress = (long)(m_Item.GetProgress () * 100.0 + 0.5);
...
{
	m_wndDialogBar.SetExtended (!m_bMilestone);
	if (!m_wndDialogBar.Create (_T(""), this, FALSE, 
			MAKEINTRESOURCE(!m_bMilestone ? IDD_GANTT_DIALOGBAR1 : IDD_GANTT_DIALOGBAR2), 
	...
	CRect rectBar;
	m_wndDialogBar.GetClientRect (rectBar);
	m_wndDialogBar.SetMinSize (rectBar.Size ());

	ShowControlBar (&m_wndDialogBar, TRUE, FALSE, TRUE);

	if (!m_bMilestone)
	{
		m_wndDialogBar.m_Progress.EnableWindow (!m_bCompleted);
		m_wndDialogBar.m_ProgressSpin.EnableWindow (!m_bCompleted);
	}
...
{
	m_wndDialogBar.UpdateData ();

	COleDateTime dtS = m_wndDialogBar.m_DateStart.GetDate ();
	COleDateTime tmS = m_wndDialogBar.m_TimeStart.GetDate ();
	...
	if (!m_bMilestone)
	{
		dtF = m_wndDialogBar.m_DateEnd.GetDate ();
		tmF = m_wndDialogBar.m_TimeEnd.GetDate ();
	}
	...
	m_Item.SetName (m_wndDialogBar.m_strName);
	...
	{
		m_Item.SetProgress (m_bMilestone ? 0.0 : (m_wndDialogBar.m_lProgress / 100.0));
	}
	...
	{
		m_Item.SetPrimaryColor (c_CategoryColors[m_nLabel]);
	}
...
void CGanttFrame::OnMarkComplete()
{
	...
	if (!m_bMilestone)
	{
		m_wndDialogBar.m_lProgress = m_bCompleted 
			? 100
		...
		m_wndDialogBar.UpdateData (FALSE);
	}
}
