...
void CProgressItem::OnDrawValue (CDC* pDC, CRect rect)
{
		...
		rectProgress.Width () > rectProgress.Height ())
	{
		...
		CMFCVisualManager::GetInstance ()->OnDrawRibbonProgressBar(pDC, &dummy, rectProgress, rectComplete, FALSE);
#else
		CBCGPRibbonProgressBar dummy;
		CBCGPVisualManager::GetInstance ()->OnDrawRibbonProgressBar(pDC, &dummy, rectProgress, rectComplete, FALSE);
#endif
	}
