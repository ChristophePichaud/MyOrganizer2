...
BEGIN_MESSAGE_MAP(CCalendarPlannerView, CBCGPPlannerManagerView)
	...
	ON_UPDATE_COMMAND_UI(ID_PLANNER_MODE_DAY, OnUpdateMode)
	ON_COMMAND(ID_PLANNER_MODE_MONTH, OnModeMonth)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_MODE_MONTH, OnUpdateMode)
	ON_COMMAND(ID_PLANNER_MODE_WEEK, OnModeWeek)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_MODE_WEEK, OnUpdateMode)
	ON_COMMAND(ID_PLANNER_MODE_WORKWEEK, OnModeWorkweek)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_MODE_WORKWEEK, OnUpdateMode)
	ON_COMMAND(ID_PLANNER_MODE_SCHEDULE, OnModeSchedule)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_MODE_SCHEDULE, OnUpdateMode)
	ON_COMMAND(ID_PLANNER_TIMELINE_60, OnTimeLine60)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_60, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_30, OnTimeLine30)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_30, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_20, OnTimeLine20)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_20, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_15, OnTimeLine15)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_15, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_10, OnTimeLine10)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_10, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_6, OnTimeLine6)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_6, OnUpdateTimeLine)
	ON_COMMAND(ID_PLANNER_TIMELINE_5, OnTimeLine5)
	ON_UPDATE_COMMAND_UI(ID_PLANNER_TIMELINE_5, OnUpdateTimeLine)
...
BOOL CCalendarPlannerView::OnPreparePrinting(CPrintInfo* pInfo)
{
	...
	if (!pInfo->m_bPreview)
#else
	if (!pInfo->m_bPreview && !pFrame->GetRibbonBar ()->IsBackstageMode ())
#endif
	{
		CPrintTypeDlg dlg (pFrame);
	...
	{
		pPrinterClass = (CRuntimeClass*)CBCGPRibbonBackstagePagePrint::s_BCGPrintInfo.lpParam;
		if (!pInfo->m_bPreview)
		{
...
void CCalendarPlannerView::OnContextMenu(CWnd*, CPoint point)
{
			 ...
			 nHit == CBCGPPlannerView::BCGP_PLANNER_HITTEST_ICON_DOWN)
	{
		...
		else if (GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_SCHEDULE)
		{
			nID = (nHit == CBCGPPlannerView::BCGP_PLANNER_HITTEST_CLIENT ||
				   nHit == CBCGPPlannerView::BCGP_PLANNER_HITTEST_HEADER)
					? IDR_PLANNER_CLIENT_SCHEDULE
					: IDR_PLANNER_TIMEBAR;
		}
...
void CCalendarPlannerView::OnModeSchedule() 
{
	SetType (CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_SCHEDULE);
}

void CCalendarPlannerView::OnUpdateMode(CCmdUI* pCmdUI) 
{
	pCmdUI->SetRadio
		(
			(pCmdUI->m_nID == ID_PLANNER_MODE_DAY && GetManagerCtrl ().GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_DAY) ||
			(pCmdUI->m_nID == ID_PLANNER_MODE_WORKWEEK && GetManagerCtrl ().GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_WORK_WEEK) ||
			(pCmdUI->m_nID == ID_PLANNER_MODE_WEEK && GetManagerCtrl ().GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_WEEK) ||
			(pCmdUI->m_nID == ID_PLANNER_MODE_MONTH && GetManagerCtrl ().GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_MONTH) ||
			(pCmdUI->m_nID == ID_PLANNER_MODE_SCHEDULE && GetManagerCtrl ().GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_SCHEDULE)
		);
}
...
void CCalendarPlannerView::OnUpdateTimeLine(CCmdUI* pCmdUI)
{
	pCmdUI->Enable (GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_DAY || 
					...
					GetType () == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_SCHEDULE);

	pCmdUI->SetRadio
		(
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_60 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_60) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_30 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_30) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_20 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_20) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_15 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_15) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_10 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_10) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_6 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_6) ||
			(pCmdUI->m_nID == ID_PLANNER_TIMELINE_5 && GetTimeDelta () == CBCGPPlannerView::BCGP_PLANNER_TIME_DELTA_5)
		);
}
												...
												BOOL bNewRecur /*= FALSE*/)
{
	...
	CAppointmentFrame dlg (*pAppEdit, pOldRule, bEcp, bNewRecur, IsReadOnly (), this);
	if (dlg.DoModal () != IDOK)
	{
		if (bAdd)
		{
...
void CCalendarPlannerView::OnKeyDownCtrl(UINT nChar, UINT /*nRepCnt*/, UINT /*nFlags*/) 
{
	...
	if (_istalpha ((TCHAR)nChar) || _istdigit ((TCHAR)nChar) || nChar == VK_RETURN)
	{
		if (GetManagerCtrl ().GetSelectedAppointmentsCount () == 1)
		{
			POSITION pos = GetManagerCtrl ().GetFirstSelectedAppointment ();
...
void CCalendarPlannerView::UpdateCalendar (const CPlannerOptions& options, BOOL bRedraw /*= TRUE*/)
{
	...
	rMan.SetViewHourInterval (options.m_ViewHourFirst, options.m_ViewHourLast, FALSE);
	...
	if (options.m_bImagesChanged)
	{
		...
		if (nBitsPerPixel > 8)
		{
			rMan.SetImages ((UINT)(options.m_DefaultImages ? 0 : IDB_PLANNER_ICONS), 
				16, (COLORREF)-1);
		}
	}
	...
	if (options.m_bUpDownIconsChanged)
	{
		(const_cast<CPlannerOptions&>(options)).m_bUpDownIconsChanged = FALSE;

		if (nBitsPerPixel > 8)
		{
			rMan.SetUpDownIcons ((UINT)(options.m_DefaultUpDownIcons ? 0 : IDB_PLANNER_UPDOWN), 
				(COLORREF)-1);
			rMan.AdjustLayout (FALSE);
		}
	}
...
void CCalendarPlannerView::UpdateCaptionText ()
{
	...
	if (pFrame != NULL)
	{
		...
		if (type == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_DAY || 
			type == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_WORK_WEEK ||
			type == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_WEEK ||
			type == CBCGPPlannerManagerCtrl::BCGP_PLANNER_TYPE_SCHEDULE)
		{
			SYSTEMTIME st;
			CString str;
			...
			if ((dtEnd - dtStart).GetDays () != 0)
			{
				...
				{
					CString strFmt (_T("d MMMM yyyy"));
					if (!bDateBeforeMonth)
					{
						strFmt = _T("MMMM d yyyy");
					}
...
void CCalendarPlannerView::Serialize(CArchive& ar)
{
	...
	else
	{
		...
		UINT nCurrentResourceID = CBCGPAppointmentBaseMultiStorage::e_UnknownResourceID;

		if (bMulti)
		{
			pStorage = DYNAMIC_DOWNCAST(CBCGPAppointmentBaseMultiStorage, rMan.GetStorage ());
			if (pStorage != NULL)
			{
				nCurrentResourceID = pStorage->GetCurrentResourceID ();
			}
		}
		...
		if (bMulti && pStorage != NULL)
		{
			...
			for (i = 0; i < dwCount; i++)
			{
				...
				rMan.AddResourceRTC (nResourceID, NULL, pInfo, TRUE, FALSE);
			}

			BOOL bResourceFound = FALSE;
			CBCGPAppointmentBaseMultiStorage::XResourceIDArray ids;
			pStorage->GetResourceIDs (ids);
			for (i = 0; i < (DWORD)ids.GetSize (); i++)
			{
				if (nCurrentResourceID == ids[i])
				{
					bResourceFound = TRUE;
					break;
				}
			...
			rMan.SetCurrentResourceID (nCurrentResourceID, FALSE);
		}
		else
		{
...
void CCalendarPlannerView::OnNcPaint()
{
