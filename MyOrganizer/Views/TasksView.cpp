...
CBCGPToolBarImages CTasksView::m_Images;
...
void CTasksView::OnInitialUpdate() 
{
	...
	if (m_bFirstTime)
	{
		...
		m_Images.Load(IDB_TASKS_ICONS);
		globalUtils.ScaleByDPI(m_Images);

#ifndef _BCGSUITE_INC_
		CBCGPToolBarImages imageHeader;
		imageHeader.SetImageSize(CSize(12, 13));
		imageHeader.Load(IDB_TASKS_HEADER32);
		globalUtils.ScaleByDPI(imageHeader);
		
		imageHeader.ExportToImageList(m_ImagesHeader);
#else
		...
		pGridCtrl->SetVisualManagerColorTheme();
		...
		if (pGridCtrl->OpenMSAccessFile (strPath))
		{
			...
			if (!lstFields.IsEmpty ())
			{
				...
				for (POSITION pos = lstFields.GetHeadPosition (); pos != NULL;)
				{
					...
					if (strField.CompareNoCase (_T("id")) != 0)
					{
...
void CTasksView::OnViewFieldchooser() 
{
	...
	m_pWndGridCtrl->ShowColumnsChooser (
		!m_pWndGridCtrl->IsColumnsChooserVisible (), TRUE);
}
...
protected:
	...
	{
		if (CTasksView::m_Images.GetCount() == 0)
		{
		...
		CTasksView::m_Images.DrawEx(pDC, rect, nValue == 1 ? 1 : 0, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
	}
};
...
protected:
	virtual void OnDrawValue (CDC* pDC, CRect rect)
	{
		...
		if (nValue != 1)
		{
			CTasksView::m_Images.DrawEx(pDC, rect, nValue == 2 ? 3 : 2, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
		}
	}
};
...
protected:
	virtual void OnDrawValue (CDC* pDC, CRect rect)
	{
		...
		if (nValue != 0)
		{
			CTasksView::m_Images.DrawEx(pDC, rect, nValue + 3, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
		}
	}
};
...
void CTasksView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
	...
	if (bActivate)
	{
		CBCGPGridCtrl* pGrid = GetGridCtrl();
		if (pGrid != NULL)
		{
			pGrid->SetVisualManagerColorTheme();
		}
