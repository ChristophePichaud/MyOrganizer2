...
void CFlagItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	if (nIndex >= 0 && nIndex < m_Icons.GetCount ())
	{
		m_Icons.DrawEx(pDC, rect, nIndex, CBCGPToolBarImages::ImageAlignHorzCenter, CBCGPToolBarImages::ImageAlignVertCenter);
	}
}
...
void CMailView::OnInitialUpdate() 
{
	...
	if (m_bFirstTime)
	{
		...
		pReportCtrl->SetVisualManagerColorTheme();
		...
		globalUtils.ScaleByDPI(m_Flags);
		...
		imageHeader.Load(IDB_MAIL_HEADER32);
		...
		pReportCtrl->SetScalingRange (0.1, 4.0);
...
void CMailView::OnViewFieldchooser() 
{
	GetReportCtrl ()->ShowColumnsChooser (
		!GetReportCtrl ()->IsColumnsChooserVisible (), TRUE);
}
