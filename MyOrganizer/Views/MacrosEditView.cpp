...
BEGIN_MESSAGE_MAP(CMacrosEditView, CBCGPEditView)
	...
	ON_COMMAND(ID_EDIT_VIEWWHITESPACE, OnEditViewWhiteSpace)
	ON_UPDATE_COMMAND_UI(ID_EDIT_VIEWWHITESPACE, OnUpdateEditViewWhiteSpace)
    ON_WM_NCPAINT()
	//}}AFX_MSG_MAP
	ON_WM_CONTEXTMENU()
...
void CMacrosEditView::OnContextMenu(CWnd*, CPoint point)
{
	...
	CPoint pt(point);
    UINT nID = IDR_EDIT_MENU;

    if (pt.x == -1 && pt.y == -1)
    {
        point = m_pEdit->GetCaretPoint();
    ...
    else
    {
	    ...
	    {
            nID = IDR_EDIT_POPUP_SELECTIONMARGIN;
	    }
    }
...
int CMacrosEditView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pEdit->SetLineNumbersMargin (TRUE, 30, 10);
...
void CMacrosEditView::OnInitialUpdate() 
{
	...
	if (m_uiXMLSettingsID != 0)
	{
		...
		else
		{
			...
			{
				LPSTR lpszXML = (LPSTR)LockResource(hGlobal);
...
{
	globalData.fontRegular.GetLogFont (&m_LogFont);
	...
	m_LogFont.lfHeight      = globalUtils.ScaleByDPI(-14);
	m_LogFont.lfWidth       = 0;
...
void CMacrosEditView::OnEditIncreaseIndent() 
{
	ASSERT_VALID (m_pEdit);
	VERIFY(m_pEdit->IndentSelection(TRUE));
}

void CMacrosEditView::OnUpdateEditIncreaseIndent(CCmdUI* pCmdUI) 
{
	ASSERT_VALID (m_pEdit);
	pCmdUI->Enable(m_pEdit->IsIndentEnabled(TRUE));
}

void CMacrosEditView::OnEditDecreaseIndent() 
{
	ASSERT_VALID (m_pEdit);
	VERIFY(m_pEdit->IndentSelection(FALSE));
}

void CMacrosEditView::OnUpdateEditDecreaseIndent(CCmdUI* pCmdUI) 
{
	ASSERT_VALID (m_pEdit);
...
{
	ASSERT_VALID (m_pEdit);
	m_pEdit->SetLineNumbersMargin (!m_pEdit->IsLineNumbersMarginVisible (), 10);
}
...
void CMacrosEditView::OnEditViewWhiteSpace()
{
	ASSERT_VALID (m_pEdit);
	m_pEdit->SetViewWhiteSpace (!m_pEdit->IsViewWhiteSpace());
}

void CMacrosEditView::OnUpdateEditViewWhiteSpace(CCmdUI* pCmdUI)
{
	ASSERT_VALID (m_pEdit);
	pCmdUI->SetCheck (m_pEdit->IsViewWhiteSpace() ? 1 : 0);
}
...
void CMacrosEditView::OnFontSize ()
{
	...
	m_LogFont.lfHeight = globalUtils.ScaleByDPI(-nSize);
...
BOOL CMacrosEditView::OnPreparePrinting(CPrintInfo* pInfo)
{
...
void CMacrosEditView::OnNcPaint()
{
