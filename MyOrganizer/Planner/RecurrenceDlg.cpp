...
BOOL CRecurrenceDlg::OnInitDialog() 
{
	...
	for (int i = 0; i < m_RulesDlg.GetSize (); i++)
	{
		...
		m_RulesDlg[i]->ModifyStyle (WS_BORDER, 0);
	}
...
void CRecurrenceDlg::OnOK() 
{
	...
	if (m_bEcp)
	{
		if (BCGPMessageBox(IDS_APPOINTMENT_5, MB_ICONWARNING | MB_OKCANCEL) == IDCANCEL)
		{
			return;
		}
	}
	...
	if (sp2 < sp1)
	{
		BCGPMessageBox(IDS_APPOINTMENT_1, MB_ICONWARNING);
		return;
	}
	...
	if (m_Dialog != NULL)
	{
		...
		{
			BCGPMessageBox(IDS_APPOINTMENT_3, MB_ICONWARNING);
			return;
		}
	}
